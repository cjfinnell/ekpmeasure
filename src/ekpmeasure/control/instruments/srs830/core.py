...
__all__ = (
    ...
    "set_internal_frequency",
    "set_internal_amplitude",
    "get_time_constant_from_frequency",
    "get_time_constant_float",
    "set_lockin_sensitivity",
    "initialize_lockin",
    "set_phase",
    "get_X_Y",
    "get_nearest_time_constant",
    "set_low_pass_filter_slope",
    "set_external_reference_slope",
    "set_signal_input_shield_grounding",
    "set_signal_input_coupling",
    "set_signal_input_configuration",
)
...
index_to_time_constant_mapper = {
    time_constant_to_index_mapper[key]: key for key in time_constant_to_index_mapper
}
...
index_to_sensitivity_mapper = {
    sensitivity_to_index_mapper[key]: key for key in sensitivity_to_index_mapper
}
...
def set_signal_input_configuration(lockin, config: str):
    ...
    if config not in set(mapper.keys()):
        raise KeyError(
            'config "{}" not allowed. Must be in "{}"'.format(
                config, set(mapper.keys())
            )
        )
...
def set_signal_input_coupling(lockin, coupling: str):
    ...
    if coupling not in set(mapper.keys()):
        raise KeyError(
            'coupling "{}" not allowed. Must be in "{}"'.format(
                coupling, set(mapper.keys())
            )
        )
...
def set_signal_input_shield_grounding(lockin, grounding: str):
    ...
    if grounding not in set(mapper.keys()):
        raise KeyError(
            'grounding "{}" not allowed. Must be in "{}"'.format(
                grounding, set(mapper.keys())
            )
        )
...
def set_external_reference_slope(lockin, slope: str):
    ...
    if slope not in set(mapper.keys()):
        raise KeyError(
            'slope "{}" not allowed. Must be in "{}"'.format(slope, set(mapper.keys()))
        )
...
def get_nearest_time_constant(time):
    ...
    time_constant = ordered_float_time_constants[ordered_float_time_constants > time][
        0
    ]  # take the first one
...
def initialize_lockin(
    ...
    assert (
        trigger == "internal" or trigger == "external"
    ), 'Trigger: {} not allowed. Must me "internal" or "external"'.format(trigger)

    if trigger == "internal" and (
        type(frequency) == type(None) or type(amplitude) == type(None)
    ):
        raise ValueError(
            "must supply a frequency if internal triggering. currently supplied None"
        )
...
def get_time_constant_float(time_constant):
    ...
    try:
        ...
        raise TypeError(
            "unable to convert {} to float. allowed suffixes are us, ms, s, ks".time_constant
        )
...
def get_time_constant_from_frequency(frequency, multiplier=3):
    ...
    tmp_out = tmp_out.split("e")[0] + scientific_str_to_time_suffix(
        "e" + tmp_out.split("e")[-1]
    )
...
def set_time_constant(lockin, time_constant):
    ...
    if time_constant not in set(time_constant_to_index_mapper.keys()):
        raise KeyError(
            "time_constant {} not allowed. allowed constants are {}".format(
                time_constant, time_constant_to_index_mapper.keys()
            )
        )
...
def set_low_pass_filter_slope(lockin, slope: str):
    ...
    if slope not in set(string_to_int_mapper.keys()):
        raise KeyError(
            'Slope "{}" not allowed. Must be chosen from "{}"'.format(
                slope, set(string_to_int_mapper.keys())
            )
        )
...
def set_reference_source(lockin, source):
    ...
    if type(source) == str:
        source = source.lower()
        assert (
            source == "internal" or source == "external"
        ), "soure is not 'internal' or 'external'. provided: {}".format(source)
        if source == "internal":
            lockin.write("fmod 1")
        if source == "external":
            lockin.write("fmod 0")
        ...
        assert (
            source == 1 or source == 0
        ), "if providing an int, must be 1 (internal) or 0 (external)"
        lockin.write("fmod {}".format(source))
    else:
        raise ValueError(
            "cannot set reference source. source {} not understood".format(source)
        )
    return
...
def set_internal_frequency(lockin, frequency):
    ...
    if type(frequency) == str:
        ...
        frequency = float(
            "{}{}".format(number, frequency_suffix_to_scientific_str(suffix))
        )
    else:
