...
__all__ = (
    ...
    "stop_pulse_gen",
    "trigger",
    "set_function_to_pulse",
    "set_run_mode_to_burst",
    "set_ncylces_for_burst_mode",
    "set_offset",
    "set_low_voltage",
    "set_high_voltage",
    "set_pulsewidth",
    ...
    "set_frequency",
    "set_pulse_delay",
    "set_function_to_ramp",
    "set_phase",
    "set_amplitude",
    "set_function_to_sine",
    "set_burst_mode_off",
)
...
def set_amplitude(pulse_gen, amplitude: str, channel=1, both=False):
    ...
    if both:
        ...
        pulse_gen.write(
            "SOURce{}:VOLTage:LEVel:IMMediate:AMPLitude {}".format(channel, amp)
        )
...
def start_pulse_gen(pulse_gen, channel=1, both=False):
    ...
    if type(channel) != int:
        raise TypeError(
            "channel must be type int. Recieved type: {}".format(type(channel))
        )
...
def set_low_voltage(pulse_gen, low_v, channel=1, both=False):
    ...
    if both:
        ...
        pulse_gen.write(
            "SOURce{}:VOLTage:LEVel:IMMediate:low {}".format(channel, low_v)
        )
...
def set_high_voltage(pulse_gen, high_v, channel=1, both=False):
    ...
    if both:
        ...
        pulse_gen.write(
            "SOURce{}:VOLTage:LEVel:IMMediate:high {}".format(channel, high_v)
        )
...
def set_offset(pulse_gen, offset="0 mV", channel=1, both=False):
    ...
    if both:
        ...
        pulse_gen.write(
            "source{}:voltage:level:IMMediate:offset {}".format(channel, offset)
        )
    return
