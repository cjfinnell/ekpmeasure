...
__all__ = (
    ...
    "set_trigger_mode",
    "set_trigger_source",
    "set_trigger_input_slope",
    "set_trigger_input_threshold",
    "check_errors",
    "clear_errors",
    "stop",
    "start",
...
def check_channel(channel: str):
    if channel.lower() not in channels:
        raise ValueError(
            'Channel "{}" not allowed. Please use one of {}'.format(channel, channels)
        )
    else:
        return channel[-1]
...
def get_pulse_mode(pg, channel: str):
    ...
    out = (
        pg.query("output{}:pulse:mode?".format(channel_int_str))
        .replace("\n", "")
        .lower()
    )
    if out not in {"single", "double", "triple", "quadruple"}:
        raise ValueError(
            "Expected .get_pulse_mode to return one of {}. Instead got {}".format(
                "'single','double', 'triple', 'quadruple'", "out"
            )
        )
    return out
...
def set_pulse_mode(pg, pulse_mode: str, channel: str = "Ch1"):
    ...
    if pulse_mode.lower() not in pulse_modes:
        raise ValueError(
            'Pulse mode "{}" not allowed. Please use one of {}'.format(
                pulse_mode, pulse_modes
            )
        )
...
def _check_pulse_count_vs_pulse_mode(pulse_count: int, pulse_mode: str):
    ...
    if pulse_count > checker_dict[pulse_mode]:
        raise ValueError(
            'Cannot modify pulse number {} in pulse mode "{}"'.format(
                pulse_count, pulse_mode
            )
        )
...
def set_pulse_width(pg, pulsewidth: str, pulse_count: int = 1, channel: str = "Ch1"):
    ...
    if pulsewidth[-2:] not in puslewidth_suffixes:
        raise ValueError(
            'Pulsewidth "{}" not allowed. Allowed are {}'.format(
                pulsewidth, puslewidth_suffixes
            )
        )
    ...
    pg.write(
        "source{}:pulse{}:width {}".format(channel_int_str, pulse_count, pulsewidth)
    )
    return
...
def set_pulse_delay(pg, delay: str, pulse_count: int = 1, channel: str = "Ch1"):
    ...
    pg.write(
        "source{}:pulse{}:delay {}".format(
            channel_int_str, pulse_count, scientific_delay
        )
    )
    return
...
def set_high_voltage(pg, high_voltage: str, channel: str = "Ch1"):
    ...
    if suffix.lower() not in {"v", "mv"}:
        raise ValueError(
            'High Voltage "{}" not allowed. Must have suffix "V or mV"'.format(
                high_voltage
            )
        )

    pg.write(
        "source{}:voltage:level:immediate:high {}".format(channel_int_str, high_voltage)
    )
    return
...
def set_low_voltage(pg, low_voltage: str, channel: str = "Ch1"):
    ...
    if suffix.lower() not in {"v", "mv"}:
        raise ValueError(
            'Low Voltage "{}" not allowed. Must have suffix "V or mV"'.format(
                low_voltage
            )
        )

    pg.write(
        "source{}:voltage:level:immediate:low {}".format(channel_int_str, low_voltage)
    )
    return
...
def set_trigger_source(pg, _type: str):
    if _type.lower() not in triggers:
        raise ValueError(
            'Trigger type "{}" not allowed, must be {}'.format(_type, triggers)
        )
...
def set_trigger_mode(pg, _mode: str):
    if _mode.lower() not in trigger_modes:
        raise ValueError(
            'Trigger mode "{}" not allowed, must be {}'.format(_mode, trigger_modes)
        )
...
def set_trigger_input_slope(pg, _type: str):
    if _type.lower() not in trigger_slopes:
        raise ValueError(
            "Trigger slope {} not allowed, must be {}".format(_type, trigger_slopes)
        )
...
def set_trigger_input_threshold(pg, threshold_voltage: str):
    ...
    if suffix.lower() not in {"v", "mv"}:
        raise ValueError(
            'Threshold Voltage "{}" not allowed. Must have suffix "V or mV"'.format(
                threshold_voltage
            )
        )
...
def symmetric_up_down_SCPI(
    pulsewidth, amplitude, offset="0mv", channel="1", *args, **kwargs
):
    """
...
def initialize_2pulse(
    ...
    assert polarity in set(
        {"up", "down"}
    ), "polarity must be 'up' or 'down'. Recieved {}".format(polarity)
    ...
    if error_check != "Error: 0, No error\n":
        ...
        raise ValueError(
            "error in initialize_2pulse init. Error Message: {}".format(error_check)
        )
