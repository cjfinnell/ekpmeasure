...
def set_horizontal_scale(inst, scale: str):
    ...
    if scale not in set(horizontal_scales.keys()):
        raise ValueError(
            'Scale"{}" not allowed, must be in {}'.format(
                scale, sorted(set(horizontal_scales.keys()))
            )
        )
...
def set_acquire_stopafter(inst, _type: str):
    ...
    if _type.lower() not in acquire_stop_after:
        raise ValueError(
            "Type {} not allowed, must be {}".format(_type, acquire_stop_after)
        )
...
def initialize_scope(
    inst, aquisition_number_pts=5000, channel="Ch3", force_init=False, **kwargs
):
    """
