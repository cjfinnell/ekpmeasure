...
def _check_channel(channel):
    ...
    try:
        assert channel.lower() in [
            "1",
            "2",
            "all",
        ], 'Channel must be in [1,2,"all"], not {}'.format(channel)
        return channel.lower()

    except AttributeError:
        assert channel in [1, 2], 'Channel must be in [1,2,"all"], not {}'.format(
            channel
        )
        return channel
...
def apply(
    inst, waveform: str, frequency: str, amplitude: str, offset: str = "0v", channel=1
):
    ...
    if channel == "all":
        ...
        inst.write(
            "SOURce{}:APPLy:{} {},{},{}".format(channel, waveform, freq, amp, offs)
        )
...
def set_output_load(agilent33500, load: str):
    ...
    if load not in set(mapper.keys()):
        raise KeyError(
            'load "{}" not allowed. Must be in "{}"'.format(load, set(mapper.keys()))
        )
