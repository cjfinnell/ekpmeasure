...
class experiment:
    ...
    def config_path(self, path, directory_delimiter="/"):
        ...
        if path[-1] != directory_delimiter:
            raise ValueError(
                'must specify a directory. "{}" does not specify a directory'.format(
                    path
                )
            )
        ...
        try:
            ...
            yn = input(
                "Path '{}' does not exist. Do you wish to create it? (y/n)".format(path)
            )
            yn = yn.lower()
    ...
    def _plot(self, data, scan_params):
        ...
        raise NotImplementedError(
            "_plot() should be overridden in experiment subclass. It is not."
        )
    ...
    def n_param_scan(
        ...
        if set(scan_param_order) != set(kw_scan_params.keys()):
            raise KeyError(
                "kw_scan_params do not have the same keys as scan_param_order"
            )
        try:
            total_scans = int(
                np.prod([len(kw_scan_params[x]) for x in kw_scan_params]) * ntrials
            )
            iteration = 0
            for key in kw_scan_params:
                ...
                if hasattr(params, "__getitem__") and type(params) != str:
                    ...
                    raise TypeError(
                        "kw_scan_param: {} must be array-like of params. it is not.".format(
                            key
                        )
                    )
            ...
            for params in iterable_param_list:
                ...
                for count in range(ntrials):
                    ...
                    print(
                        "Scan {} of {}. {}".format(
                            iteration, total_scans, current_scan_params
                        )
                    )

                    trial_df = trial(
                        self.run_function, kwargs, self.path, return_df=True
                    )
                    if plot:
...
def trial(
    run_function, run_function_args, path, return_df=False, save_meta_data_csv=True
):
    """
    ...
    try:
        ...
        save_name = input(
            "there was an error generating a save name: {}\n please enter a unique name".format(
                e
            )
        )
        trial = np.nan
    ...
    assert type(df) == type(
        pd.DataFrame()
    ), "run_function {} does not return a pandas.DataFrame as its third return argument, it must".format(
        run_function.__name__
    )
    ...
    try:
        ...
        if set(meta_data.columns) != set(existing_meta_data.columns):
            raise ValueError(
                "the columns of meta_data do not match the existing columns of the data in this path ({}). Please ensure you are producing data of the same type, or move to a new path. Please note, your data was saved with file complete filename: {}, but it was not added to meta_data".format(
                    path, path + save_name
                )
            )
