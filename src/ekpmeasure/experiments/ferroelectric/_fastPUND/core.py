...
def _get_frequency(pw, delay):
    ...
    desired_frequency = np.round(
        1 / (4 * float(str(n) + universal.time_suffix_to_scientic_str(s))), 2
    )
...
def _add_time_strings(str1, str2):
    ...
    float_total = n1 * float(
        "1" + universal.time_suffix_to_scientic_str(s1)
...
def _add_voltage_strings(str1, str2):
    ...
    float_total = n1 * float("1" + universal.voltage_amp_mapper[s1]) + n2 * float(
        "1" + universal.voltage_amp_mapper[s2]
    )
...
def _config_bk(
    pg, pw: str, delay: str, voltage: str, offset: str, inverted: bool = False
):
...
def run_function(
    ...
    voltage: str,
    offset: str,
    offset_time_delay: str = "90ns",
    identifier: str = "none",
    inverted: bool = False,
    **kwargs
):

    base_name = "FastPUND_{}_{}_{}_{}_{}".format(
        identifier, voltage, offset, pulsewidth, delay
    )
