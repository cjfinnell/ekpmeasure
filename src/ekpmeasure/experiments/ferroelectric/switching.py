...
from ...control.instruments.tektronixTDS6604 import (
    initialize_scope as initialize_scope_tds6604,
)
...
identifier_to_diameter_dict = dict(
    {
        "4": 4,
        "6": 6,
        "8": 8,
...
def single_pulse_run_function(
    ...
    if scopetype != "6604" and scopetype != "620B":
        raise ValueError(
            'scopetype must be "6604" or "620B". Recieved {}'.format(scopetype)
        )
    ...
    try:
        ...
        try:
            ...
            raise ValueError(
                "unable to interpret pulsewidth {}, interpreted as {}".format(
                    pulsewidth, pw
                )
            )
    ...
    try:
        ...
        try:
            ...
            raise ValueError(
                "unable to interpret delay {}, interpreted as {}".format(delay, dlay)
            )
    ...
    if not test:
        ...
        for pulsen in [1, 2]:
            ...
            if scopetype == "6604":
                ...
                raise ValueError(
                    "Please check scopetype. Set to {}, which is not allowed".format(
                        scopetype
                    )
                )
        ...
        meta_data.update(
            {
                ...
                "area": np.pi
                * (identifier_to_diameter_dict[identifier.split("um")[0]] / 2) ** 2,
            }
        )
    except KeyError:
        print("unable to extract diameter and area from key {}".format(identifier))
        meta_data.update({"diameter": np.nan, "area": np.nan})

    return save_base_name, meta_data, df


def two_pulse_run_function(
    pg,
    scope,
    identifier,
    pulsewidth,
...
def apply_preset_pulse(
    ...
    **kwargs
    ...
    """
    assert pulse_polarity in set(
        {"up", "down"}
    ), "pulse_polarity must be 'up' or 'down' not '{}'".format(pulse_polarity)
...
def preset_run_function(
    ...
    area="fromdiameter",
    diameter="fromidentifier",
    preset_pulsewidth="100ns",
    preset_voltage="3000mv",
    scope_channel="Ch1",
    test=False,
):
    """
    ...
    if preset_pulsewidth[-1] != "s":
        raise ValueError(
            "preset_pulsewidth {} is not supported please use ns, us, ms, s suffix".format(
                preset_pulsewidth
            )
        )
    ...
    if not test:
        ...
        for pulsen in [1, 2]:
            ...
            apply_preset_pulse(
                ...
                preset_pulsewidth,
                preset_voltage,
                pulse_polarity=preset_pulse_polarity,
            )
            time.sleep(0.8)
            initialize_2pulse(
                pg,
    ...
    save_presetpulsewidth = (
        str(
            float(
                preset_pulsewidth.replace("ns", "e-9")
                .replace("us", "e-6")
                .replace("ms", "e-3")
                .replace("s", "")
            )
