...
def drive_and_collect(
    ...
    base_name_adder = [
        "{}{}".format(key, wf_params[key]).replace(".", "p") for key in wf_params
    ]
    ...
    data = r.data.rename(
        columns={"AOUT_0": "DriveVoltage(V)", "AIN_1": "SenseVoltage(V)"}
    ).drop(columns=["AOUT_1", "AIN_0"])
    data["SenseVoltage(V)"] = data["SenseVoltage(V)"] / (
        termination_ohm * amplification_level
...
class RUN:
    ...
    def load_waveform(self, wvfrm_vstack):
        ...
        self.input_wfm_df = pd.DataFrame(
            {i: wvfrm_vstack[i, :] for i in range(wvfrm_vstack.shape[0])}
        )
    ...
    def go(self, **kwargs):
        ...
        for i in range(self.nave):
            returned = apply_and_listen(
                self.wf_1d,
                self.nzeros_front,
                self.nzeros_back,
                in_channel_start=self.in_channel_start,
                in_channel_end=self.in_channel_end,
                out_channel_start=self.out_channel_start,
                out_channel_end=self.out_channel_end,
                quiet=self.quiet,
                **kwargs
            )
            (
            ...
            try:
                ...
                for i in range(0, count_in):
                    out.append(
                        ul.to_eng_units(self.board_num, self.ul_range, data_array_in[i])
                    )
                out = np.array(out)
