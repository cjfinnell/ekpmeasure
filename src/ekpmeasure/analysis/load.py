...
def load_Dataset(path, meta_data=None):
    ...
    if len(existing_ekpds) != 0:
        warnings.showwarning(
            "There exist .ekpds files ({}) in this directory. If you want to load those Datasets, be sure to use ``.read_ekpds``".format(
                existing_ekpds
            ),
            UserWarning,
            "",
...
def read_ekpds(filename):
    ...
    for x in preamble_spl:
        if "pointercolumn" in x:
            ...
            if type(y) == type(None):
                ...
                raise ValueError(
                    "error in load. expected 2 items in preamble, recieved at least 3. This likely originated from a change in .to_ekpds without appropriate changes to reading ekpds."
                )
    ...
    return Dataset(
        ...
        readfileby,
        pointercolumn=preamble_dict["pointercolumn"],
    )
    ...
    if type(meta_data) == type(None):
        try:
            ...
            try:
                ...
                print(
                    'No file named "meta_data" exists in path "{}", you may want to create one or both of these with `.generate_meta_data()`'.format(
                        path, path
                    )
                )
                return pd.DataFrame()
            ...
            try:
                ...
                print(
                    'Pickle file "meta_data" exists in path "{}", but failed to load (likely do to incompatible pandas versions). Backup, "meta_data.csv" does not exist in "{}", you may want to create one or both of these with `.generate_meta_data()`'.format(
                        path, path
                    )
                )
                return pd.DataFrame()
...
def generate_meta_data(
    path, mapper, pass_path=False, pointercolumn="filename", overwrite=False
):
    """
    ...
    if "meta_data" in set(os.listdir(path)):
        if not overwrite:
            yn = input(
                "this path ({}) already has meta_data, do you wish to recreate it? (y/n)".format(
                    path
                )
            )
            if yn.lower() != "y":
    ...
    for file in os.listdir(path):
        try:
            ...
            continue
        try:
            existing_meta_data = pd.concat(
                [existing_meta_data, meta_data], ignore_index=True
            )
        except NameError:
    ...
    if pointercolumn not in set(existing_meta_data.columns):
        warnings.showwarning(
            'there is no map to key "{}" in mapping function "{}" provided\nEnsure self.pointercolumn property has been set appropriately or else you will be unable to retrieve data'.format(
                pointercolumn, mapper.__name__
            ),
            SyntaxWarning,
            "",
            0,
