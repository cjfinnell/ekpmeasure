...
def load_radiant_loop_from_text_file(
    file, measured_value="Charge", return_meta_data=False, delimiter=","
):
    raise NameError(
        '"load_radiant_loop_from_text_file" is deprecated. Please use "read_loop_txt"'
    )
...
def read_loop_txt(file, measured_value="Charge", return_meta_data=False, delimiter=","):
    ...
    if measured_value.lower() not in set({"charge", "polarization", "current"}):
        raise ValueError(
            'measured_value {} not supported. must be either "charge" or "polarization"'.format(
                measured_value
            )
        )
    ...
    for line in lines:
        ...
        if (
            len(spl) > 1
        ):  # often there is a comma before the actual data. why? who knows! this is the case where there exists such a comma
    ...
    for row in data:
        ...
        if (
            len(spl) <= 1
        ):  # there are some rows in here which are not data and they are therefor not comma sep
            continue
    ...
    if measured_value.lower() == "charge":
        out["MeasuredPolarization"] = (
            out["MeasuredPolarization"] * float(meta_data["SampleArea(cm2)"]) * 1e6
        )  # to convert from uC to pC
        out.rename(columns={"MeasuredPolarization": "MeasuredCharge(pC)"}, inplace=True)
    elif measured_value.lower() == "polarization":
        out.rename(
            columns={"MeasuredPolarization": "MeasuredPolarization(uC/cm2)"},
            inplace=True,
        )
    if return_meta_data:
