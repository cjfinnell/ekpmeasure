...
def _get_starttime_from_startarg_1d(time, start_arg):
    ...
    if len(time.shape) > 1:
        raise ValueError(
            "time must be 1dimensional. currently has shape {}".format(time.shape)
        )
...
def reset_time(data_dict, key="p1", cutoff=0.01, grace=10):
    ...
    """
    assert set({"p1", "p2", "time"}).issubset(
        set((data_dict.keys()))
    ), "data_dict keys ({}) do not match required keys: {}".format(
        set(data_dict.keys()), set({"p1", "p2", "time"})
    )
    ...
    assert key in set(
        {"p1", "p2"}
    ), "key {} is not in allowed. must be 'p1' or 'p2'".format(key)
    assert key in set(
        data_dict.keys()
    ), "key {} does not exist in data_dict keys ({})".format(key, data_dict.keys())
    ...
    tmpout = {
        "time": data_array_builder(),
        "p1": data_array_builder(),
        "p2": data_array_builder(),
    }

    for time, p1, p2 in zip(time_ida, p1_ida, p2_ida):
        assert (
            time.shape == p1.shape and time.shape == p2.shape
        ), "shapes do not match: time - {}, p1 - {}, p2 - {}".format(
            time.shape, p1.shape, p2.shape
        )
        if key == "p1":
            checker = p1
        elif key == "p2":
...
def smooth(data_dict, key="dp", N=3, Wn=0.05):
    ...
    """
    assert key in set(data_dict.keys()), "key {} is does not exist in data_dict".format(
        key
    )
    out = data_dict.copy()
...
def subtract_median_of_lastN(data_dict, key="dp", N=20):
    ...
    (to_subtract,) = _fod_dimensionality_fixer(
        data_dict, check_key=key, keys_to_fix=[key]
    )
...
def get_saturation_and_switching_time(
    data_dict,
    key="int",
    n_points_for_saturation=50,
    top_percent=90,
    bottom_percent=10,
):
    ...
    intdp, time = _fod_dimensionality_fixer(
        data_dict, check_key=key, keys_to_fix=[key, "time"]
    )
    ...
    for d in range(ndims):
        try:
            ...
            arg_at_bottomsat = np.argwhere(
                X > saturation * bottom_percent / 100
            ).flatten()[0]
...
def integrate(data_dict, key="dp"):
    ...
    assert "time" in set(
        data_dict.keys()
    ), "data_dict must contain key 'time'. It does not. Keys are {}".format(
        data_dict.keys()
    )
...
def get_pol_trans_from_dps(
    data_dict, area="from diameter", diameter=None, time_unit="ns", **kwargs
):
    """
    ...
    returns:
        (dict): dict with keys 'polarization' and 'time'. 'polarization' is
            ...
            ```
    """
    assert time_unit in set(
        {"ns", "us"}
    ), "time_unit {} not allowed. Allowed time_unit(s) are 'us' and 'ns'".format(
        time_unit
    )
    assert "dp" in set(
        data_dict.keys()
    ), "data_dict must contain key 'dp'. It does not. Keys are {}".format(
        data_dict.keys()
    )
    assert "time" in set(
        data_dict.keys()
    ...
    if type(area) == str:
        if area == "from diameter" and type(diameter) == type(None):
            raise ValueError(
                "Neither area nor diameter was supplied. Did you forget to pass the data definition?"
            )
        elif area != "from diameter":
            raise ValueError(
                "'{}' not allowed for area. Must be float, int, or 'from diameter'".format(
                    area
                )
            )
        else:
        ...
        if len(area) > 1:
            raise ValueError(
                "More than one area supplied!!! This is likely because you passed the definition as area and the data is not grouped by a single area capacitor size."
            )
        else:
            area = float(area[0])
    else:
    ...
    time_unit_multiplier = {"ns": 0.001, "us": 1}
...
def get_polarization_transients_from_dps(data_dict):
    ...
    warnings.showwarning(
        "get_polarization_transients_from_dps is deprecated. See integrate() instead. Consider using get_pol_trans_from_dps()",
        DeprecationWarning,
        "",
        0,
    )
