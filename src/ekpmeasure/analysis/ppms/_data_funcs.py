...
def window(data_dict, key="Y", window_size=5, interval=[0, 270]):
    ...
    angle_centers = [
        window_size * i + window_size / 2 - interval[0]
        for i in range(int((interval[1] - interval[0]) / window_size))
    ]
    windows = [
        (center - window_size / 2, center + window_size / 2) for center in angle_centers
    ]
...
def center_yaxis(data_dict, key="Y", top_percentile=90, bottom_percentile="symmetric"):
    ...
    for row in ida:
        center = np.mean(
            (np.percentile(row, top_percentile), np.percentile(row, bottom_percentile))
        )
...
def fit_sine(data_dict, anglekey="angle", key="Y", periodicity=1, units="degrees"):
    ...
    if units != "degrees" and units != "radians":
        raise ValueError(
            "units must be either degrees or radians. Not {}".format(units)
        )
    ...
    return {
        "params": out_params.build(),
        "fakex": out_fake_angle.build(),
        "simulated": out_simulation.build(),
    }
...
def average_over_same_angle(
    data_dict, key, centers_every=10, tolerance=2, ignore_first_n=100, ignore_end_n=0
):
    """
