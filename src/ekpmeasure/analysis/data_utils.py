...
__all__ = (
    "get_vals_by_definition",
    "vals_by_definition_to_2darray",
)
...
def get_vals_by_definition(data, definition_key, data_key):
    ...
    for i in data:
        ...
        if len(defn[definition_key]) != 1:
            raise ValueError(
                "definition_key {} returned more than one value for data index {} with definition\n{}".format(
                    definition_key, i, defn
                )
            )
...
def vals_by_definition_to_2darray(
    vals_by_definition, converter="Default", ascending=True
):
    ...
    if not hasattr(converter, "__call__"):
        raise TypeError(
            "Converter must be a function. Got type {}".format(type(converter))
        )
