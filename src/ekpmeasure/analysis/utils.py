...
def merge_Datas(tpl, by: str):
    ...
    for i, b in enumerate(by_options):
        ...
        for dat in tpl:
            ...
            if len(tdat) > 1:
                raise ValueError(
                    "At least one Data has more than one index corresponding to condition: data.contains({{'{}':[{}]}}). Try grouping before matching?".format(
                        by, b
                    )
                )
...
def merge_Datasets(datasets):
    raise NameError(
        'merge_Datasets was deprecated following version 0.1.4. Use "concat_Datasets" instead! (It has the exact some functionality)'
    )
...
def concat_Datasets(datasets):
    ...
    for dset in datasets[1:]:
        if dset.readfileby != readfileby:
            raise ValueError(
                "not all datasets have the same function for readfileby. Ensure they agree and try again."
            )
    ...
    for i, dset in enumerate(datasets):
        if i == 0:
            ...
            new_df = pd.concat(
                (new_df, pd.DataFrame(dset.meta_data)), ignore_index=True
            )
