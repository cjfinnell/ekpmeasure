...
def construct_Dataset_from_dataframe(function):
    ...
    def wrapper(*args, **kwargs):
        ...
        new_index_to_path = pd.Series(
            {
                i: index_to_path[map_new_index_to_old_index[i]]
                for i in map_new_index_to_old_index
            }
        )
        ...
        return Dataset(
            path=new_path, initializer=dataframe, readfileby=args[0].readfileby
        )
...
def _convert_ITP_to_path_to_index(index_to_path):
    ...
    return {
        x: y.values
        for x, y in pd.Series(index_to_path.index, index=index_to_path.values).groupby(
            level=0
        )
    }
...
class Dataset:
    ...
    def __init__(
        self, path, initializer, readfileby=pd.read_csv, pointercolumn="filename"
    ):
    ...
    def summary(self):
        ...
        for column in self.columns:
            if column == self.pointercolumn:
                continue
            summary.update(
                {column: _remove_nans_from_set(set(self.meta_data[column].values))}
            )
    ...
    def filter_on_column(self, column, function, **kwargs_for_function):
        ...
        return self.meta_data[
            self.meta_data[column].apply(function, **kwargs_for_function).values
    ...
    def _write_ekpds_file(self, filename):
        preamble = "pointercolumn:{}|path:{}".format(
            self.pointercolumn, {key: list(self.path[key]) for key in self.path}
        )
    ...
    def _construct_index_to_path(self, path, initializer):
        ...
        if type(path) != dict:
            ...
            index_to_path = pd.Series(
                {i: path for i in range(len(self.meta_data))}, dtype="object"
            )
        else:
            for l, key in enumerate(path):
                # path is {path:[indices]}, need inverse
                if l == 0:
                    index_to_path = pd.Series({i: key for i in path[key]})
                else:
                    # do not ignore index
    ...
    def remove_index(self, index):
        ...
        path = _convert_ITP_to_path_to_index(
            self.index_to_path.drop(index=index).reset_index(drop=True)
        )
    ...
    def remove_nonexistent_files_from_metadata(self):
        ...
        for path in paths:
            ...
            pointercolumn_values_for_path = self.meta_data.iloc[indices_to_check][
                self.pointercolumn
            ].values
    ...
    def _group(self, by, level=None):
        ...
        for ijk, key in enumerate(groups):
            ...
            for index in original_dataset_indices:
                original_row = self.meta_data.loc[
                    index
                ]  # this is a pandas series of a row from the original dataset
                # columns in this row
                if type(new_row) == type(None):
                    # import pdb; pdb.set_trace()
                    new_row = {
                        col: dict({index: original_row[col]})
            ...
            for col in new_row:
                if col != self.pointercolumn:
                    new_row[col] = set(
                        list([new_row[col][x] for x in new_row[col].keys()])
                    )
    ...
    def save_meta_data(
        ...
        if len(set(self.index_to_path)) != 1:
            raise ValueError(
                "Dataset does not contain a unique path to data. Saving of this type of Dataset is not supported."
            )
    ...
    def _set_path_absolute(self):
        ...
        if type(self.path) == str:
            ...
            raise TypeError(
                "Only str or dict supported as self.path. Please report this issue."
            )
    ...
    def get_data(
        ...
        if type(groupby) == type(None):
            data_to_retrieve = self._group(
                by=None, level=0
            )  # gives us a unique col for each
        else:
        ...
        for counter, i in enumerate(data_to_retrieve.index):  # for each row
            ...
            for k, index_of_original in enumerate(filename_index_to_path_dict):
                try:
                    tdf = readfileby(
                        self.index_to_path[index_of_original]
                        + filename_index_to_path_dict[index_of_original]
                    )
                except Exception as e:
                    raise Exception(
                        "error reading data. ensure self.readfileby is correct and that readfileby returns a pandas dataframe. self.readfileby is currently set to {}.\nError was: {}".format(
                            self.readfileby.__name__, e
                        )
                    )
                ...
                if set(tdf.columns) != columns_set:
                    raise ValueError(
                        "not all data in this Dataset has the same columns!"
                    )

                if k == 0:  # build the internal data out
                    internal_out = {
                        "definition": {
                            col: data_to_retrieve.at[i, col] for col in self.columns
                        },
                    ...
                    try:  # catch ValueError if the concatenation fails for having different lengths. Allows us to merge data of different lengths
                        for col in columns_set:
                            internal_out["data"].update(
                                {
                                    col: np.vstack(
                                        (internal_out["data"][col], tdf[col].values)
                                    )
                                }
                            )
                            ...
                            for col in columns_set:
                                ...
                                if len(current_stack.shape) != 2:
                                    if len(current_stack.shape) == 1:
                                        current_stack = np.reshape(
                                            current_stack, (1, len(current_stack))
                                        )
                                    else:
                                        raise ValueError(
                                            "Current vstack is not 2 dimensional, meaning each row contains at least 2D data. Merging non-matching shapes is not allowed at higher dimensionality."
                                        )
                                ...
                                for index in range(current_nrows):
                                    ...
                                    row = np.concatenate(
                                        (
                                            row,
                                            np.array(
                                                [np.nan for i in range(n_nans_to_add)]
                                            ),
                                        )
                                    )
                                    if index == 0:
                                ...
                                to_stack_on_end = np.concatenate(
                                    (
                                        new_data,
                                        np.array(
                                            [
                                                np.nan
                                                for i in range(new_len - len(new_data))
                                            ]
                                        ),
                                    )
                                )
                                internal_out["data"].update(
                                    {col: np.vstack((new_stack, to_stack_on_end))}
                                )
                        else:
                            raise (e)
...
def _summarize_data(data):
    ...
    for index in data:
        ...
        for key in defn:
            try:
                out[key].update(
                    _remove_nans_from_set(set({value for value in defn[key]}))
                )
            except KeyError:
                out.update(
                    {key: _remove_nans_from_set(set({value for value in defn[key]}))}
                )
    return out
...
class Data:
    ...
    def _get_indices_satisfying_definition_condtion(self, condition):
        ...
        for key in condition:
            if not hasattr(condition[key], "__iter__"):
                raise AttributeError(
                    'iterable not provided as value in condition arg. value associated with key "{}" is type {}'.format(
                        key, type(condition[key])
                    )
                )

            if type(condition[key]) == str:
                raise TypeError(
                    "condition value must not be str. check key: {}.".format(key)
                    + " try {"
        ...
        for condition_key in condition:
            for index in self._dict:
                ...
                if _check_definition_contains_or(
                    defn, condition_key, condition[condition_key]
                ):
    ...
    def filter(
        self,
        data_condition_function_dict,
        definition_condition_dict=None,
        additional_data_keys_to_filter=None,
    ):
        """Filter the data.
        ...
        if type(additional_data_keys_to_filter) == type(None):
            ...
            additional_data_keys_to_filter = np.array(
                [additional_data_keys_to_filter]
            ).flatten()

        assert (
            len(data_condition_function_dict) <= 1
        ), "more than one condition supplied in data_condition_function_dict. only one is supported at a time."
        data_function_key = list(data_condition_function_dict)[0]
        assert hasattr(
            data_condition_function_dict[data_function_key], "__call__"
        ), "value associated with key '{}' in data_condition_function_dict is a not a function. is type {}".format(
            data_condition_function_dict,
            type(data_condition_function_dict[data_function_key]),
        )
        assert data_function_key in set(
            self._dict[0]["data"].keys()
        ), "data_function_key '{}' not in data. available keys are {}".format(
            data_function_key, set(self[0]["data"].keys())
        )

        sat_indices = self._get_indices_satisfying_definition_condtion(
            definition_condition_dict
        )
        ...
        for index in sat_indices:
            ...
            for key in keys_to_update:
                ...
                if old_shape != checker.shape:
                    raise ValueError(
                        'data corresponding to data_key "{}" does not have the same shape as data corresponding to data_function_key "{}"'.format(
                            key, data_function_key
                        )
                    )
        ...
        for key in self._dict.keys():
            try:
                ...
                if pass_defn:
                    ...
                    overlap = set(kwargs.keys()).intersection(
                        set(self._dict[key]["definition"].keys())
                    )
                    if len(overlap) != 0:
                        raise ValueError(
                            'kwargs passed in both definition and as kwargs in .apply(). Overlapping keys are "{}"'.format(
                                overlap
                            )
                        )
                    ...
                    internal_out = {
                        ...
                        "data": data_function(
                            ...
                            defn=self._dict[key]["definition"],
                            **to_pass
                        ),
                    }
                else:
                ...
                if ignore_errors:
                    print(
                        "Error in data_function: {} \n{}".format(
                            data_function.__name__, e
                        )
                    )
                    print(
                        "Skipping data key: {} with defintion: \n{}".format(
                            key, self._dict[key]["definition"]
                        )
                    )
                else:
                    raise e
    ...
    def to_DataFrame(self, how="lump_mean", include_defn_keys=[], defn_converter=None):
        ...
        if len(include_defn_keys) != len(float_converter):
            raise ValueError(
                "Number of float converters does not equal number of definition keys. Must supply equal number. i.e. len(float_convert) must be equal to len(include_defn_keys)"
            )
        ...
        for key in data_keys:
            ...
            if type(_dict_for_key) is not dict:
                out[key] = [
                    how(_dict_for_key, key)
                ]  # the case when we have only one index in Data
            else:
                # import pdb; pdb.set_trace()
        ...
        for key, converter in zip(include_defn_keys, float_converter):
            ...
            if type(defn) is not dict:
                out[key] = [converter(_get_unique_definition_value_for_key(defn, key))]
            else:
                out[key] = [
                    converter(_get_unique_definition_value_for_key(defn[i], key))
                    for i in defn
                ]
    ...
    def plot(
        ...
        **kwargs
        ...
        if cmap not in set(cm.cmaps_listed.keys()):
            raise KeyError(
                'cmap "{}" not supported. Supported colormaps are {}'.format(
                    cmap, cm.cmaps_listed.keys()
                )
            )

        if color == None:
            colors = [
                cm.cmaps_listed[cmap](x)
                for x in np.linspace(0, 1, len(self._dict.keys()))
            ]
        else:
        ...
        for color, index in zip(colors, self._dict.keys()):
            ...
            for plotkey in data_keys_to_plot:
                ...
                for i in range(to_plot.shape[0]):
                    if (
                        i == 0
                    ):  # label only the first becuase we don't want a big legend with many trials
                        if x == None:
            ...
            for plotkey in data_keys_to_plot:
                ...
                for i in range(to_plot.shape[0]):
                    if (
                        ...
                        if x == None:
                            ax.scatter(
                                to_plot[i, :], color=color, label=label, **kwargs
                            )
                        else:
                            ax.scatter(
                                xs[i, :],
...
class _data_sorter:
    ...
    def sort(self, key=None, reverse=False):
        ...
        for i in self._keys:
            ...
            assert (
                len(definition_value) == 1
            ), "Definition for index '{}' is not unique. It contains multiple values: {}. Cannot sort.".format(
                i, definition_value
            )
...
def _get_unique_definition_value_for_key(definition_dict, key):
    ...
    if len(_set) != 1:
        raise ValueError(
            'Values corresponding to key "{}" are not singular and unique. Instead recieved "{}"'.format(
                key, _set
            )
        )
