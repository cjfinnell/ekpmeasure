...
__all__ = (
    ...
    "current_suffix_to_scientific_str",
...
def scientific_str_to_time_suffix(sci_str):
    ...
    returns:
        (str): Time suffix
    """
    assert sci_str in set(
        sci_to_time_mapper.keys()
    ), "sci_str {} not in sci_to_time_mapper. Allowed keys are {}".format(
        sci_str, list(sci_to_time_mapper.keys())
    )
...
def time_suffix_to_scientic_str(time_suffix):
    ...
    """
    assert time_suffix in set(
        time_to_sci_mapper.keys()
    ), "time_suffix {} not in time_to_sci_mapper. Allowed keys are {}".format(
        time_suffix, list(time_to_sci_mapper.keys())
    )
...
def voltage_suffix_to_scientic_str(volt_suffix):
    ...
    returns:
        (str): Scientific notation str
    """
    assert volt_suffix in set(
        voltage_amp_mapper.keys()
    ), "volt_suffix '{}' not in voltage_amp_mapper. Allowed keys are {}".format(
        volt_suffix, list(voltage_amp_mapper.keys())
    )
...
def frequency_suffix_to_scientific_str(freq_suffix):
    ...
    assert freq_suffix in set(
        freq_mapper.keys()
    ), "Suffix '{}'' not in freq_mapper. Allowed keys are {}".format(
        freq_suffix, list(freq_mapper.keys())
    )
...
def current_suffix_to_scientific_str(current_suffix):
    ...
    returns:
        (str): Scientific notation str
    """
    assert current_suffix in set(
        current_amp_mapper.keys()
    ), "Suffix '{}'' not in current_amp_mapper. Allowed keys are {}".format(
        current_suffix, list(current_amp_mapper.keys())
    )
...
def _scientific_notation(number):
    ...
    if len(str(number).split("e")) > 1:
        ...
        if before_decimal[0] == "0" and len(before_decimal) == 1:
            ...
            out = "{}.{}e{}".format(
                before_decimal[0], before_decimal[1:] + after_decimal, exponent
            )
    return out
